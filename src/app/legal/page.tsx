...
export default function LegalHub() {
    ...
    // VIEW STATE: 'hub' (Grid) or 'editor' (The Drafting Tool)
    ...
    const handleCardClick = (doc: any) => {
        // 1. Check Permissions
        if (!hasAccess(doc.tiers)) return; // Logic handled in UI (redirects to pricing)

        // 2. If it's the Tenancy Agreement, open the Editor
        if (doc.title === "Standard Tenancy Agreement") {
            setView('editor');
            return;
        }

        // 3. For others, simulate download (Demo Mode)
    ...
    const documents = [
        {
            ...
            type: "Residential",
            tiers: ['Free', 'Pro', 'Premium', 'Gold', 'Diamond']
        },
        {
            id: 2,
            ...
            type: "Corporate",
            tiers: ['Pro', 'Premium', 'Gold', 'Diamond']
        },
        {
            id: 3,
    ...
    // ==========================================
    // VIEW 1: THE EDITOR (Your Provided Code)
    // ==========================================
    if (view === 'editor') {
        return (
            <div className="min-h-screen bg-[#FAFAF9] dark:bg-[#0F172A] text-[#0F172A] dark:text-white font-sans flex flex-col transition-colors duration-500">
                ...
                <header className="h-16 border-b border-gray-200 dark:border-gray-800 px-6 flex items-center justify-between bg-white dark:bg-[#1E293B] z-10 sticky top-0 transition-colors">
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => setView('hub')} // GO BACK TO HUB
                            className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors text-[#0F172A] dark:text-white"
                        >
                            <ArrowLeft size={20} />
                        </button>
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden md:overflow-hidden overflow-y-auto">
                    ...
                    <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1E293B] p-6 md:p-8 overflow-y-auto flex flex-col h-auto md:h-full shrink-0 transition-colors">
                        <div className="mb-8">
                            <div className="w-12 h-12 bg-blue-50 dark:bg-blue-900/30 rounded-xl flex items-center justify-center text-blue-900 dark:text-blue-400 mb-4">
                                <PenTool size={24} />
                            </div>
                        ...
                        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white w-full md:w-[595px] min-h-[600px] md:min-h-[842px] shadow-2xl p-8 md:p-12 relative text-[10px] leading-relaxed text-justify text-gray-800 font-serif">
                            ...
                            {!hasDraft && !isDrafting ? (
                                <div className="h-full flex flex-col items-center justify-center text-gray-300 space-y-4">
                                    <FileText size={64} />
                                    <p className="font-sans text-sm text-center">Enter details to generate a draft</p>
                                </div>
                            ) : (
                                <div className={isDrafting ? 'blur-sm transition-all' : ''}>
                                    <div className="text-center mb-8 border-b-2 border-[#0F172A] pb-4">
                                        <h1 className="text-xl md:text-2xl font-bold text-[#0F172A] mb-1">TENANCY AGREEMENT</h1>
                                        <p className="text-gray-500 font-sans text-xs">Prepared by Belmont Legal Shield AI</p>
                                    </div>
                                    <div className="space-y-4 text-xs">
                                        ...
                                        <div className="mt-16 flex justify-between gap-4">
                                            <div className="text-center w-1/2"><div className="w-full h-0.5 bg-gray-300 mb-2"></div><p>Signed by Landlord</p></div>
                                            <div className="text-center w-1/2"><div className="w-full h-0.5 bg-gray-300 mb-2"></div><p>Signed by Tenant</p></div>
                                        </div>
                                    </div>
                                    {hasDraft && (
                                        <motion.div initial={{ scale: 2, opacity: 0, rotate: -20 }} animate={{ scale: 1, opacity: 1, rotate: -12 }} transition={{ type: "spring", stiffness: 200, damping: 10 }} className="absolute bottom-12 right-12 md:bottom-24 md:right-24 border-4 border-green-600 text-green-600 rounded-full w-24 h-24 md:w-32 md:h-32 flex items-center justify-center font-bold text-center p-2 opacity-80">
                                            <div className="border border-green-600 rounded-full w-20 h-20 md:w-28 md:h-28 flex flex-col items-center justify-center">
                                                <CheckCircle2 size={24} />
                                                <span className="text-[8px] md:text-[10px] uppercase mt-1">Belmont Legal</span>
                                                <span className="text-xs md:text-sm font-black">VERIFIED</span>
                                                <span className="text-[8px]">{new Date().toLocaleDateString()}</span>
                                            </div>
                                        </motion.div>
                                    )}
                                </div>
                            ...
                            {isDrafting && (
                                <div className="absolute inset-0 flex items-center justify-center bg-white/50 z-20">
                                    <div className="bg-[#0F172A] text-white px-6 py-3 rounded-full flex items-center gap-3 shadow-xl">
                                        <PenTool size={18} className="animate-bounce" />
                                        <span>Writing Clauses...</span>
                                    </div>
                                </div>
                            )}
                        </motion.div>
                        {hasDraft && (
                            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="absolute bottom-8">
                                <button className="px-8 py-3 bg-[#D4AF37] text-[#0F172A] font-bold rounded-full shadow-lg hover:scale-105 transition-transform flex items-center gap-2">
        ...
        <div className="min-h-screen bg-[#FAFAF9] dark:bg-[#0F172A] p-8 font-sans transition-colors duration-500 pb-32">
            <div className="max-w-6xl mx-auto mb-12">
                ...
                <p className="text-gray-500 dark:text-gray-400 max-w-xl">
                    Generate legally binding real estate documents instantly.
                    Your <strong>{user?.tier || 'Free'} Tier</strong> grants you specific access rights.
                </p>
            </div>
                        ...
                        <div key={doc.id} className={`p-8 rounded-3xl border transition-all duration-300 relative overflow-hidden ${locked ? 'bg-gray-50 dark:bg-[#1E293B]/50 border-gray-200 dark:border-gray-800 opacity-70' : 'bg-white dark:bg-[#1E293B] border-gray-100 dark:border-gray-700 shadow-lg hover:border-[#D4AF37]'}`}>
                            ...
                            {locked ? (
                                ...
                                <button
                                    ...
                                    disabled={generating === doc.title}
                                    className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 text-white transition-all ${completed === doc.title ? 'bg-green-600' : 'bg-[#0F172A] dark:bg-white dark:text-[#0F172A] hover:bg-[#D4AF37] dark:hover:bg-[#D4AF37] hover:text-white'}`}
                                >
                                    {generating === doc.title ? (<><Loader2 className="animate-spin" /> Generating...</>) : completed === doc.title ? (<><CheckCircle2 /> Download Ready</>) : (
                                        // IF IT IS TENANCY AGREEMENT, SHOW 'OPEN EDITOR', ELSE 'GENERATE'
